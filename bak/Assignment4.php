...
<body>
	...
	try {
        ...
        if(empty($username) && empty($password)) {
				...
				$insertNewUser = "INSERT INTO userprofile (UserID, ProfileID) 
								SELECT user.UserID, profile.ProfileID 
								FROM user, profile 
								WHERE user.Username = '$newusername' AND profile.FirstName = '$first'";
				$results = mysql_query($insertNewUser)
        			or die('could not insert into database: ' . mysql_error());		
	...
	?>
	<br>
	<a href="Assignment4.html"> Back to Login Page </a>
</body>
</html>